...
function LoginSignup() {
  ...
  {
    e.preventDefault();
    let item = {fullname, username, password, email}
    console.log(item);

    try {
      const response = await fetch('https://bdcoetask2.vercel.app/api/v1/users/register', {
        mode: 'no-cors',
        method: 'POST', 
        headers: {
      ...
      <div className="inputs">
        ...
        <div className="submit-container">
          <div
            ...
            onClick={() => {
              action === "Sign Up"? signup : setAction("Sign Up")
            }}
          >
            <input type="submit" value="Sign Up" className="signup-login" />
          </div>
          <div
